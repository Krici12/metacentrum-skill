...
If you already have the public key saved in your `~/.ssh/known_hosts` file and the frontend comes up with a different one, you will see a message similar to the following:
...
- Go to [PBSmon page with host SSH key fingerprints](https://metavo.metacentrum.cz/ssh_known_hosts.fp). This page is managed by our admins. It contains fingerprints of SSH keys of all our hosts (computing nodes as well as frontends).
...
- If this string is the same as the one displayed in the warning message (here `:bZfXg8ZTXR3KlUNbJ8rhMP0QOtbGdp7sMPCW22bIE0M`), then the key is genuine and you can go on with removing the old key and changing it to the new one

- If the strings are different, do not log in and contact our user support.

In case you need to see a list of public keys of all hosts, there is also [a complete list of public keys](https://metavo.metacentrum.cz/ssh_known_hosts) available.

To make your PC forget the old SSH host key, remove the old fingerprint from your local known_hosts file. Open your Terminal and run:
...
Replace [hostname_or_IP] with the server address you are trying to connect to (e.g., `ssh-keygen -R skirit.metacentrum.cz`).
